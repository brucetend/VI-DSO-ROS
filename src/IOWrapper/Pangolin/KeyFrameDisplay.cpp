...
bool KeyFrameDisplay::refreshPC(bool canRefresh, float scaledTH, float absTH, int mode, float minBS, int sparsity)
{
	...
	for(int i=0;i<numSparsePoints;i++)
	{
		...
		for(int pnt=0;pnt<patternNum;pnt++)
		{
			...
			//cout << ((originalInputSparse[i].u+dx)*fxi + cxi) * depth <<endl;
			//根据深度估计来获得三维投影...
			//ROS_INFO("hahahah %f", ((originalInputSparse[i].u+dx)*fxi + cxi) * depth);
			// ROS_INFO("X: %f", ((originalInputSparse[i].u+dx)*fxi + cxi) * depth);
			// ROS_INFO("Y: %f", ((originalInputSparse[i].v+dy)*fyi + cyi) * depth);
			// ROS_INFO("Z: %f", ((originalInputSparse[i].u+dx)*fxi + cxi) * depth);

			position_x = ((originalInputSparse[i].u+dx)*fxi + cxi) * depth;
			position_y = ((originalInputSparse[i].u+dy)*fyi + cyi) * depth;
			position_z = depth*(1 + 2*fxi * (rand()/(float)RAND_MAX-0.5f));
