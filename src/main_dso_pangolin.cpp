...
public:
	...
	//对于posecallback的参数应该是什么？
	void imageprocess(cv_bridge::CvImagePtr cv_ptr, double time);
...
:it_(nh)
{
	position_publisher = nh.advertise<geometry_msgs::Point>("pose",1);
	...
	image_sub_ = it_.subscribe("usb_cam/image_raw",1,&Detector::imageCallback,this);
	
	ROS_INFO("I see:****************************************");
	//ros::spin(); 
	// spin就是一直运行直到，按下control C
...
void Detector::imageprocess(cv_bridge::CvImagePtr cv_ptr, double time)
{
	int frameID = 0;
	Undistort* undistorter = 0;
	FullSystem* fullSystem = 0;
	std::cout<<"%s calib0"<<std::endl;
    undistorter = Undistort::getUndistorterForFile(calib0, gammaCalib, vignette);

	MinimalImageB minImg((int)cv_ptr->image.cols, (int)cv_ptr->image.rows,(unsigned char*)cv_ptr->image.data);
	ImageAndExposure* undistImg = undistorter->undistort<unsigned char>(&minImg, 1,0, 1.0f);
	
	undistImg->timestamp=time;   // relay the timestamp to dso
...
void Detector::imageCallback(const sensor_msgs::ImageConstPtr& tem_msg)
{
	...
	//fullheight = cv_ptr->image.rows;
     //process(cv_ptr->image);
      ...
      image = cv_ptr->image;
	  ROS_INFO("I see:hahaahah****************************************");
      imshow("teng", image); 


	  //imageprocess(cv_ptr, tem_msg->header.stamp.toSec());
}

// void Detector::poseCallback()
// {
...
// 	    cv::waitKey(-1);
    ...
    if(viewer != 0)
        //viewer->run();
        viewer->run(det.position_publisher);
